...
class TestDumpWithPyTorch:
    ...
    def test_dump_with_torch_tensor(self, tmp_path):
        ...
        mock_tensor.detach.return_value.cpu.return_value.numpy.return_value = mock_numpy_result
...
class TestTqdmWithoutPackage:
    ...
    def test_tqdm_wrap_without_tqdm(self, tmp_path):
        ...
        with patch("builtins.__import__", side_effect=ImportError("No module named 'tqdm'")):
    ...
    def test_tqdm_wrap_with_mock_tqdm(self, tmp_path):
        ...
        with patch.dict("sys.modules", {"tqdm": mock_tqdm_module, "tqdm.auto": mock_tqdm_module.auto}):
            meta = TensorMeta("test", "F32", (10,), 0, 40)
            writer = StreamWriter.open(tmp_path / "test.st", [meta])
...
class TestDumpMixedTensors:
    ...
    def test_dump_mixed_torch_numpy(self, tmp_path):
        ...
        mock_torch_tensor.detach.return_value.cpu.return_value.numpy.return_value = mock_numpy_result
        ...
        with patch("stsw._core.dtype.normalize", side_effect=lambda x: "F32" if x == mock_torch_tensor.dtype else "I32"):
            dump(state_dict, output_path)
